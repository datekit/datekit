...
import { startOfMonth, endOfMonth, startOfWeek, endOfWeek, setMonth, format, isSameDay } from 'date-fns';
...
export type View = typeof DAY_VIEW | typeof WEEK_VIEW | typeof MONTH_VIEW | typeof YEAR_VIEW;
...
export function generateWeekView(selectedDate: Date): Week {
  ...
  const firstDayOfWeek = new Date(selected.getFullYear(), selected.getMonth(), selected.getDate() - selected.getDay());
  const lastDayOfWeek = new Date(selected.getFullYear(), selected.getMonth(), selected.getDate() + (6 - selected.getDay()));
...
export function generateMonthView(selectedDate: Date): Month {
  ...
  for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
    ...
    const events: any = []; // Add logic to fetch events for the day
    days.push({ date: new Date(date), isCurrentMonth, isToday, isSelected, events });
  }

  const monthName = new Date(selected.getFullYear(), selected.getMonth()).toLocaleString('default', { month: 'long' });
