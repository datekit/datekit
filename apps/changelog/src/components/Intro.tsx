...
export function Intro() {
  const documentationUrl = process.env.NEXT_PUBLIC_DOCUMENTATION_URL || 'https://docs.datekit.dev';
  return (
    <>
      <div>
        <Link href="/">
      ...
      <h1 className="mt-14 font-display text-4xl/tight tracking-tighter font-semibold text-white">
        Beautiful calendar for{' '}
        <span className="text-rose-400">React & Nest.js</span>
      </h1>
      <p className="mt-4 text-sm/6 text-stone-300">
        Datekit is a beautiful calendar component for React & Nest.js. It is designed to be simple, flexible, and easy
        to use.
      </p>
      <div className="mt-8 flex flex-wrap justify-center gap-x-1 gap-y-3 sm:gap-x-2 lg:justify-start">
        <IconLink href={documentationUrl} icon={BookIcon} className="flex-none">
          Documentation
        </IconLink>
        <IconLink href="https://github.com/datekit/datekit" target="_blank" icon={GitHubIcon} className="flex-none">
          GitHub
        </IconLink>
        <IconLink href="/feed.xml" target="_blank" icon={FeedIcon} className="flex-none">
          RSS
        </IconLink>
      </div>
    </>
...
export function IntroFooter() {
  return (
    <p className="flex items-baseline gap-x-2 text-[0.8125rem]/6 text-stone-500">
      Brought to you by{' '}
      <IconLink href="https://github.com/foxted" icon={GitHubIcon} compact>
        foxted
      </IconLink>
    </p>
  );
