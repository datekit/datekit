  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied
          ? 'bg-rose-400/10 ring-1 ring-inset ring-rose-400/20'
    ...
    >
      <span
        aria-hidden={copied}
        className={clsx(
          'pointer-events-none flex items-center gap-0.5 text-stone-400 transition duration-300',
          copied && '-translate-y-1.5 opacity-0',
        )}
      >
        <ClipboardIcon className="h-5 w-5 fill-stone-500/20 stroke-stone-500 transition-colors group-hover/button:stroke-stone-400" />
        Copy
      </span>
      <span
        aria-hidden={!copied}
        className={clsx(
          'pointer-events-none absolute inset-0 flex items-center justify-center text-rose-400 transition duration-300',
          !copied && 'translate-y-1.5 opacity-0',
        )}
      >
        Copied!
...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y border-b-white/7.5 border-t-transparent bg-white/2.5 bg-stone-900 px-4 dark:border-b-white/5 dark:bg-white/1">
      {tag && (
...
function CodeGroupHeader({
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b border-stone-700 bg-stone-800 px-4 dark:border-stone-800 dark:bg-transparent">
      {title && (
...
export function CodeGroup({
  ...
  let containerClassName =
    'my-6 overflow-hidden rounded-2xl bg-stone-900 shadow-md dark:ring-1 dark:ring-white/10'
  let header = (
